...
export default function FindRides() {
  ...
  const rides = useMemo(() => {
    ...
    if (bands.size > 0) list = list.filter((r) => bands.has(bandOf(r.departTime)));
    ...
    if (sort === "fastest")
      list.sort((a, b) => parseTime(a.arriveTime) - parseTime(a.departTime) - (parseTime(b.arriveTime) - parseTime(b.departTime)));
    return list;
  ...
  return (
    <main className="mx-auto max-w-7xl px-6 py-10 md:py-14">
      <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <p className="text-[11px] uppercase tracking-widest font-black text-secondary">Available journeys</p>
          ...
          <p className="text-muted-foreground mt-2">
            Showing {rides.length} premium intercity {rides.length === 1 ? "ride" : "rides"} for {dateLabel}
          </p>
        </div>
          ...
          {(["cheapest", "earliest", "fastest"] as Sort[]).map((s) => (
            <button
              ...
              className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all ${
                sort === s ? "bg-surface-lowest shadow-ambient text-primary" : "text-secondary hover:text-foreground"
              }`}
            >
              {s}
            </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <aside className="lg:col-span-3 space-y-6">
          <div className="bg-surface-low rounded-2xl p-6 space-y-8">
                  ...
                  return (
                    <button
                      ...
                      className={`flex flex-col items-center justify-center p-3 rounded-xl transition-all ${
                        active ? "bg-gradient-primary text-primary-foreground shadow-ambient" : "bg-surface-lowest text-secondary hover:text-foreground"
                      }`}
                    >
                ...
                {[1, 2, 3, 4].map((n) => (
                  <button
                    ...
                    className={`h-10 w-10 rounded-full text-sm font-black transition-all ${
                      selectedSeats === n ? "bg-gradient-primary text-primary-foreground shadow-ambient" : "bg-surface-lowest text-foreground hover:bg-surface-high"
                    }`}
                  >
                    {n}
                    {n === 4 && "+"}
            ...
            <FilterBlock title="Match quality">
              <div className="space-y-3">
                <Toggle label="Reverse leg matched only" checked={matchedOnly} onChange={setMatchedOnly} />
                <Toggle label="Verified drivers (4.7+)" checked={verifiedOnly} onChange={setVerifiedOnly} />
              </div>
            </FilterBlock>
          </div>
          ...
          <div className="bg-gradient-primary rounded-2xl p-6 text-primary-foreground space-y-3">
            ...
            <h3 className="font-display text-lg font-bold tracking-tight">Every driver, verified.</h3>
        ...
        <div className="lg:col-span-9 space-y-5">
          {rides.length === 0 ? (
            <div className="bg-surface-lowest rounded-2xl p-12 text-center shadow-ambient">
              ...
              <p className="text-muted-foreground mt-2 text-sm">Loosen your time band or reduce the seat count.</p>
            </div>
          ) : (
...
function FilterBlock({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <h3 className="text-[11px] font-black uppercase tracking-widest text-foreground mb-4">{title}</h3>
      {children}
    </div>
  );
}
...
function Toggle({ label, checked, onChange }: { label: string; checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <label className="flex items-center justify-between gap-3 cursor-pointer group">
      <span className="text-sm font-medium text-secondary group-hover:text-foreground transition-colors">{label}</span>
      <button
        type="button"
        onClick={() => onChange(!checked)}
        className={`relative h-6 w-11 rounded-full transition-colors shrink-0 ${checked ? "bg-gradient-primary" : "bg-surface-high"}`}
        aria-pressed={checked}
      >
        <span className={`absolute top-0.5 h-5 w-5 rounded-full bg-surface-lowest shadow-ambient transition-transform ${checked ? "translate-x-5" : "translate-x-0.5"}`} />
      </button>
    </label>
  );
}
