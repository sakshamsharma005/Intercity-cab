...
export default function PostRide() {
  ...
  const [form, setForm] = useState({
    from: "", to: "", date: "", departTime: "", returnDate: "", returnTime: "",
    car: "", seats: 3, price: 1000,
  });
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitted(true);
    setTimeout(() => {
      const params = new URLSearchParams({ from: form.from, to: form.to, date: form.date, seats: "1" });
  ...
  if (submitted) {
    return (
      <main className="mx-auto max-w-2xl px-6 py-24 text-center">
        ...
        <h1 className="font-display text-4xl font-black tracking-tighter">Trip posted. Matching now.</h1>
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-10 md:py-14">
      <div className="mb-10">
        <p className="text-[11px] uppercase tracking-widest font-black text-secondary">For drivers</p>
        ...
        <p className="text-muted-foreground mt-3 max-w-2xl">
          Tell us where and when you're driving. We'll auto-match passengers for your outbound journey and your empty return.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          <Section title="Vehicle & seats" icon={<Car className="h-4 w-4" />}>
            ...
            <div className="mt-4">
              <p className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground mb-2">Seats available</p>
                ...
                {[1, 2, 3, 4, 5, 6].map((n) => (
                  <button
                    ...
                    className={`h-10 w-10 rounded-full text-sm font-black transition-all ${
                      form.seats === n ? "bg-gradient-primary text-primary-foreground shadow-ambient" : "bg-surface-low text-foreground hover:bg-surface-high"
                    }`}
                  >
                    {n}
                  </button>
          ...
          <Section title="Price per seat" icon={<IndianRupee className="h-4 w-4" />}>
            ...
            <input type="range" min={300} max={5000} step={50} value={form.price} onChange={(e) => setForm({ ...form, price: Number(e.target.value) })} className="w-full mt-4 accent-primary" />
        ...
        <aside className="lg:sticky lg:top-24 self-start space-y-4">
          <div className="bg-surface-lowest rounded-2xl p-6 shadow-ambient space-y-5">
            <div className="flex items-center gap-2">
              ...
              <p className="text-[11px] uppercase tracking-widest font-black text-primary">Live match estimate</p>
            </div>
            <div>
              <p className="font-display text-3xl font-black tracking-tighter">
                {form.from || "From"} <span className="text-muted-foreground font-normal">→</span> {form.to || "To"}
              </p>
            ...
            {matchEstimate ? (
              <div className="space-y-3 pt-2">
                ...
                <Stat label="Projected revenue" value={`₹${Math.round(matchEstimate.revenue).toLocaleString("en-IN")}`} highlight />
              </div>
            ) : (
              <p className="text-sm text-muted-foreground">Add your route to see a match forecast.</p>
            )}

            <button type="submit" className="w-full bg-gradient-primary text-primary-foreground rounded-xl px-6 py-3.5 font-bold text-sm hover:shadow-lift transition-all active:scale-[0.98]">
  ...
  return (
    <section className="bg-surface-lowest rounded-2xl p-6 md:p-7 shadow-ambient">
      <div className="flex items-center gap-2 mb-5">
        <div className="h-8 w-8 rounded-lg bg-surface-low flex items-center justify-center text-primary">{icon}</div>
...
function Stat({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="flex items-center justify-between">
      <span className="text-xs uppercase font-bold tracking-widest text-muted-foreground">{label}</span>
      <span className={`font-display font-black tabular-nums ${highlight ? "text-primary text-xl" : "text-foreground text-base"}`}>{value}</span>
    </div>
  );
}
